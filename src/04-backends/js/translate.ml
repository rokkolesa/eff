...
and of_computation { it; _ } =
  ...
  | CoreSyntax.Let (p_c_lst, c) ->
      let to_bind abs acc =
        ...
        | _ -> Bind (term, (_match, Sequence (projections @ [ Return acc ])))
      in
...
and of_abstraction ?(_match_gen = None) abs =
  ...
  match projections with
  | [] -> (_match, term)
  | _ -> (_match, Sequence (projections @ [ Return term ]))
...
and of_abstraction2 (p1, p2, c) =
  ...
  | projections ->
      (_match1, _match2, Sequence (projections @ [ Return (of_computation c) ]))
